...
# flaskla nginx dockerda uyusmuyordu session hatasi cozumu
# Set to True if i start using HTTPS
app.config['SESSION_COOKIE_SECURE'] = False
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'
app.config['PERMANENT_SESSION_LIFETIME'] = 1800000
app.config['SESSION_TYPE'] = 'filesystem'  # Use filesystem instead of signed cookies
app.config['SECRET_KEY'] = 'a_strong_secret_key'  # Make sure you have a strong secret key
...
def history():
    try:
        ...
        query = """
            SELECT s.songname, p.session_date, sg.SingerName
            FROM picked_items p
            JOIN songs s ON p.picked_song = s.Songid
            Join singer sg on s.SingerID=sg.SingerID
            WHERE p.userid = %s
