function resmidegistir() {
  ...
  const selectedVideo = select.value; // Seçilen videonun URL'si
  const selectedId = select.options[select.selectedIndex].id; // Seçilen videonun id'si
  const sessionDate = const sessionDate = new Date();
  const localDate = sessionDate.toLocaleString("en-GB", {
    hour12: false,
    timeZone: "Europe/Istanbul"  // Burada "Europe/Istanbul" yerel saat diliminizi kullanabilirsiniz
  });

  console.log(selectedId);
  if (selectedVideo) {
    videoSource.setAttribute("src", selectedVideo);
    videoElement.load(); // Video yüklenmesini başlat

    // Video yükleme işlemi tamamlandıktan sonra fetch isteği yapılacak
    videoElement.onloadeddata = function () {
      fetch("/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          pickedsong: selectedId, //videonun idsi 
          session_date: sessionDate,
        }),
      })
        .then((response) => response.json())
        .then((data) => {
          console.log("Response:", data); // Veritabanına başarıyla kaydedildi mi kontrol et
        })
