...
function resmidegistir() {
  ...
  const selectedSongId = selectedOption.value; // Burada song id var (örneğin: 5)
  const selectedVideo = selectedOption.dataset.src; // Burada video src var
  const videoElement = document.getElementById("resimid");
  const videoSource = document.getElementById("videosource");
  const sessionDate = new Date().toISOString();

  if (selectedVideo && selectedSongId) {
    ...
    fetch("/history", {
      ...
      body: JSON.stringify({
        pickedsong: selectedSongId,
        session_date: sessionDate,
      }),
    })
      .then((res) => res.json())
      .then((data) => console.log("Gönderim başarılı:", data))
