...
def history():
    try:
        ...
        if request.method == "POST":
            ...
            # Extract song ID from path or use a lookup query to convert path to ID
            # Option 1: Extract from path (if format is consistent)
            # song_id = picked_song.split('/')[-1].split('.')[0]  # Example extraction

            # Option 2: Use the full path directly (and modify your join accordingly)
            # This requires changing your SQL query below to match the full path
        ...
        # GET isteği: Kullanıcının geçmişini getiriyoruz
        cursor = dbconnection.cursor(dictionary=True)
        # Kullanıcı ID'sini session'dan alıyoruz
        user_id = session.get('userid')

        # Option 1: If you're storing paths but want to join with song_id
        # This would require you to extract the song ID from the path somehow

        # Option 2: Modify the join condition to match what you're storing
        query = """
            SELECT p.picked_song, s.singerid, p.session_date 
            FROM picked_items p
            INNER JOIN songs s ON p.picked_song LIKE CONCAT('%%', s.Songid, '%%')
            WHERE p.userid = %s
            ORDER BY p.session_date DESC;
        """
        # Alternative option: Just show history without the join
        # query = """
        #     SELECT picked_song, session_date
        #     FROM picked_items
        #     WHERE userid = %s
