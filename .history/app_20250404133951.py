...
@login_required
def history():
    data = request.json  # JSON verisini request.body'den alıyoruz
    ...
    try:
        ...
        if request.method == "POST":
            ...
            song = data.get('pickedsong')
            session_date = data.get('session_date')

            query = "INSERT INTO picked_items (userid, song, session_date) VALUES (%s, %s, %s)"
            cursor.execute(query, (user_id, song, session_date))
            dbconnection.commit()
            cursor.close()
            return jsonify({"success": True, "message": "History updated successfully"}), 200

        # GET isteği: Kullanıcının geçmişini getir
        user_id = session.get('userid')  #  session for user ID
        query = """
            SELECT p.picked_song, s.singerid, p.session_date 
            FROM picked_items p
            INNER JOIN songs s ON p.picked_song = s.Songid
            WHERE p.userid = %s
