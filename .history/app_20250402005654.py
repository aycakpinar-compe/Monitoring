...
def signin():
    try:
        ...
        query = "SELECT id, username, userpass FROM user WHERE useremail = %s"
...
def signup():
    ...
    if request.method == "POST":
        try:
            ...
            cursor = dbconnection.cursor(dictionary=True)
            check_query = "SELECT id FROM user WHERE useremail = %s"
            ...
            cursor.execute(insert_query, (username, useremail,
...
def history():
    try:
        ...
        if request.method == "POST":
            data = request.get_json()
            user_id = session.get('user_id')  # Use session for user ID
            song = data.get('song')
